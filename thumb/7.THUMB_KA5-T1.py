...
layout = {
    ...
    "left_margin_text": 50,    # lề trái khi vẽ text
    "margin_x": 40,            # khoảng cách 2 bên text
    "bar_padding": 30,         # khoảng cách chữ hook tới viền pill
    "top_padding": 170,          # khoảng cách text tới lề trên
    ...
    "highlight_color": "#ec0505",     # màu highlight trong text (vàng sáng)
    "keyword_color": "#ffffff",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 0,              # độ dày viền chữ hook
    ...
    "hook_bg_color": "#ec0505",           # nền pill hook (vàng)
    "hook_edge_color": "#ce0611",         # viền pill hook (xanh dương đậm)
    "hook_text_color": "#ffffff",         # màu chữ hook (đỏ tươi)
    "hook_text_stroke_color": "#f9e1e1",  # màu viền chữ hook (hồng nhạt)

    # 🖌️ Tùy chọn render chữ hook trực tiếp ở đây
    "render_hook_text_here": True,
    "hook_bg_enabled": True,
}



FONT_PATH = BASE_DIR.parent / "fonts" / "00034-UTM-AvoBold.ttf"
KEY_FONT_PATH = BASE_DIR.parent / "fonts" / "00034-UTM-AvoBold.ttf"
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA5-T1.jpg"
    try:
    ...
    if best_font:
        ...
        # 2) Vẽ chữ hook (chỉ chữ; không nền/viền/bóng nếu hook_bg_enabled=False)
        txt_w = int(draw.textlength(kw_text, font=kw_font))
        bbox  = kw_font.getbbox(kw_text)
        txt_h = bbox[3] - bbox[1]

        # Vẽ chữ hook một lần (tránh vẽ 2 lần chồng nhau)
        draw.text(
            (kw_x, kw_y),
            kw_text,
        ...
        # Nếu vẫn muốn có khung chữ nhật (pill) thì bật lại hook_bg_enabled=True
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA5-T1", na=False)]
