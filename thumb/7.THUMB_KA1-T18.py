# thumbnail_generator.py – QUY HOẠCH FULL (có viền đen cho TEXT THUMB)
...
layout = {
    ...
    "top_padding": 30,         # khoảng cách text tới lề trên
    ...
    # 🎨 Màu sắc cơ bản
    "text_bg_color": "#ffde59",
    "text_color": "#ffde59",
    "highlight_color": "#c1ff72",
    "keyword_color": "#141414",
    "keyword_stroke": 1,
    "keyword_stroke_color": "#141414",

    # 🖌️ Viền cho TEXT THUMB (mới)
    "text_stroke": 6,                # độ dày viền chữ text thumb
    "text_stroke_color": "#000000",  # màu viền (đen)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#ffde59",
    "hook_edge_color": "#ffde59",
    "hook_text_color": "#141414",
    "hook_text_stroke_color": "#141414",

    # 🖌️ Tùy chọn render chữ hook trực tiếp ở đây
    "render_hook_text_here": True,
}

FONT_PATH = BASE_DIR.parent / "fonts" / "Zuume SemiBold.ttf"
KEY_FONT_PATH = BASE_DIR.parent / "fonts" / "Zuume SemiBold.ttf"
...
def draw_rich_line(draw, x, y, segments, font):
    sw = layout.get("text_stroke", 0)
    sf = layout.get("text_stroke_color", "#000000")
    for seg, is_high in segments:
        ...
        draw.text(
            ...
            fill=color,
            font=font,
            stroke_width=sw,        # viền cho TEXT THUMB
            stroke_fill=sf,         # màu viền
        )
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA1-T18.jpg"
    try:
    ...
    for ln in lines:
        ...
        draw_rich_line(draw, x, y, segs, font)  # vẽ text có stroke
        y += font.size + dyn_sp
        glb += len(ln) + 1
    ...
    if best_font:
        ...
        # 2) Vẽ chữ (đặt trước để căn pill chuẩn theo kích thước text)
        draw.text(
            (kw_x, kw_y),
            kw_text,
            fill=layout["hook_text_color"],
            font=kw_font,
            anchor="mm",
            stroke_width=layout["keyword_stroke"],
            stroke_fill=layout["hook_text_stroke_color"],
        )

        # Tạo pill bao quanh chữ (giới hạn trong panel trái)
        ...
        if hasattr(draw_ctx, "rounded_rectangle"):
            draw_ctx.rounded_rectangle([x0, y0, x1, y1], radius=RADIUS,
                                       outline=HOOK_EDGE, width=EDGE_W, fill=HOOK_BG)
        else:
            draw_ctx.rectangle([x0, y0, x1, y1], outline=HOOK_EDGE, width=EDGE_W, fill=HOOK_BG)

        # Nếu muốn vẽ chữ sau cùng (đè lên), bật flag dưới:
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA1-T18", na=False)]
