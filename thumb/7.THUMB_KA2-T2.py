...
layout = {
    ...
    "highlight_color": "#ffd631",     # màu highlight trong text (vàng sáng)
    "keyword_color": "#cf0404",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 1,              # độ dày viền chữ hook
    "keyword_stroke_color": "#f9e1e1",# màu viền chữ hook (hồng nhạt)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#f9e1e1",           # nền pill hook (vàng)
    "hook_edge_color": "#cf0404",         # viền pill hook (xanh dương đậm)
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA2-T2.jpg"
    try:
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA2-T2", na=False)]
