...
layout = {
    ...
    "text_color": "#fffeff",          # màu chữ thường (trắng)
    "highlight_color": "#ffd300",     # màu highlight trong text (vàng sáng)
    "keyword_color": "#000000",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 1,              # độ dày viền chữ hook
    "keyword_stroke_color": "#000000",# màu viền chữ hook (hồng nhạt)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#ffed00",           # nền pill hook (vàng)
    "hook_edge_color": "#ffed00",         # viền pill hook (xanh dương đậm)
    "hook_text_color": "#000000",         # màu chữ hook (đỏ tươi)
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA1-T17.jpg"
    try:
    ...
    if best_font:
        kw_font = best_font
        kw_x = panel_w // 2
        OFFSET_Y = 20   # khoảng cách từ lề dưới
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA1-T17", na=False)]
