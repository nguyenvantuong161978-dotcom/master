...
layout = {
    ...
    "text_bg_color": "#005d9e",   # NỀN TEXT THUMB
    "text_color": "white",        # Màu chữ trên nền xanh đậm
    "highlight_color": "#fff500", # HIGHLIGHT → vàng
    "bar_color": "#fff500",       # NỀN HOOK → vàng
    "keyword_color": "#cf0404",   # CHỮ HOOK → đỏ
    "keyword_stroke": 0,          # Viền mỏng hơn
...
def generate(row):
    ...
    # Crop về bounding box (bỏ vùng transparent)
    if cut.mode == "RGBA":
        alpha = cut.split()[-1]
        bbox = alpha.getbbox()
        if bbox:
    ...
    # Tô nền vùng text (5/6)
    draw.rectangle([(0, 0), (panel_w, text_h)], fill=layout["text_bg_color"])
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA4-T3", na=False)]
