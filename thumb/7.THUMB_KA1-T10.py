...
layout = {
    ...
    "text_color": "#ff66c4",          # màu chữ thường (trắng)
    "highlight_color": "#ffffff",     # màu highlight trong text (vàng sáng)
    "keyword_color": "#141414",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 1,              # độ dày viền chữ hook
    "keyword_stroke_color": "#141414",# màu viền chữ hook (hồng nhạt)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#ff66c4",           # nền pill hook (vàng)
    "hook_edge_color": "#ff66c4",         # viền pill hook (xanh dương đậm)
    "hook_text_color": "#141414",         # màu chữ hook (đỏ tươi)
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA1-T10.jpg"
    try:
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA1-T10", na=False)]
