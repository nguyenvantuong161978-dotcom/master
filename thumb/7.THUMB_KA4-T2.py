...
def generate(row):
    ...
    # --- Keyword bar ---
    # --- Keyword bar ---
    draw.rectangle([(0, bar_top), (panel_w, img_h)], layout["bar_color"])

    kw_text = clean_text(row.hook).upper()
    max_w_bar = panel_w - 2 * layout["bar_padding"]
    
    # ❗ Giảm 10% chiều cao bar để tránh text đụng mép
    safe_ratio = 0.7
    max_h_bar = int(bar_h * safe_ratio)

    # Fit font sao cho KHÔNG vượt max_w_bar và max_h_bar
    best_font = None
    for sz in range(layout["keyword_max_font"], layout["keyword_min_font"] - 1, -2):
        font = ImageFont.truetype(str(KEY_FONT_PATH), sz)
        text_w = draw.textlength(kw_text, font=font)
        bbox = font.getbbox(kw_text)
        text_h = bbox[3] - bbox[1]
        if text_w <= max_w_bar and text_h <= max_h_bar:
            best_font = font
            break
    ...
    if best_font:
        ...
        draw.text((kw_x, kw_y), kw_text, fill=layout["keyword_color"], font=kw_font,
                  anchor="mm", stroke_width=layout["keyword_stroke"], stroke_fill=layout["keyword_stroke_color"])
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA4-T2", na=False)]
