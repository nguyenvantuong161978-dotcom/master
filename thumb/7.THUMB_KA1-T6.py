...
layout = {
    # ⚙️ Cấu hình kích thước thumbnail
    "img_width": 1920,         # chiều rộng ảnh thumbnail (px)
    "img_height": 1080,        # chiều cao ảnh thumbnail (px)
    "right_prop": 0.35,        # tỉ lệ panel bên phải dành cho nhân vật (35%)
    "right_panel_left_margin": 40, 
    "left_margin_text": 20,    # lề trái khi vẽ text
    "margin_x": 40,            # khoảng cách 2 bên text
    "bar_padding": 20,         # khoảng cách chữ hook tới viền pill
    "top_padding": 5,          # khoảng cách text tới lề trên
    "bottom_padding": 5,       # khoảng cách text tới lề dưới
    "line_spacing": 10,        # khoảng cách giữa các dòng text
    "fade_width": 40,          # độ rộng fade khi làm mờ biên
    "min_font": 36,            # cỡ chữ nhỏ nhất cho text chính
    "max_font": 388,           # cỡ chữ lớn nhất cho text chính
    "keyword_min_font": 28,    # cỡ chữ nhỏ nhất cho hook
    "keyword_max_font": 200,   # cỡ chữ lớn nhất cho hook
    "min_bar_frac": 1 / 6,     # chiều cao tối thiểu của bar hook (so với ảnh)
    "max_bar_frac": 1 / 6,     # chiều cao tối đa của bar hook (so với ảnh)

    # 🎨 Màu sắc cơ bản
    "text_bg_color": "#005d9e",       # nền panel text bên trái (xanh dương đậm)
    "text_color": "#000000",          # màu chữ thường (trắng)
    "highlight_color": "#f10000",     # màu highlight trong text (vàng sáng)
    "keyword_color": "#f10000",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 15,              # độ dày viền chữ hook
    "keyword_stroke_color": "#ffffff", # màu viền chữ hook (hồng nhạt)
    # Viền cho text thumb (chữ thường)
    "text_stroke_enabled": False,
    "text_stroke_width": 15,          # dày viền chữ chính
    "text_stroke_color": "#ffffff",  # viền trắng
    "highlight_stroke_color": "#ffffff",  # viền cho phần highlight (nên cùng màu trắng)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#f9e1e1",           # nền pill hook (vàng)
    "hook_edge_color": "#e9ca46",         # viền pill hook (xanh dương đậm)
    "hook_text_color": "#f10000",         # màu chữ hook (đỏ tươi)
    "hook_text_stroke_color": "#000000",  # màu viền chữ hook (hồng nhạt)

    # 🖌️ Tùy chọn render chữ hook trực tiếp ở đây
    "render_hook_text_here": True,
    "hook_bg_enabled": False,
}
...
def draw_rich_line(draw, x, y, segments, font):
    # cấu hình stroke
    stroke_on = layout.get("text_stroke_enabled", False)
    sw = layout.get("text_stroke_width", 0) if stroke_on else 0
    text_stroke = layout.get("text_stroke_color", "#ffffff")
    hi_stroke   = layout.get("highlight_stroke_color", text_stroke)

    for seg, is_high in segments:
        fill = layout["highlight_color"] if is_high else layout["text_color"]
        if sw > 0:
            # vẽ có stroke
            draw.text(
                (x, y),
                ...
                font=font,
                stroke_width=sw,
                stroke_fill=(hi_stroke if is_high else text_stroke),
            )
        else:
            # vẽ thường
            draw.text((x, y), seg, fill=fill, font=font)

        # lưu ý: textlength không tính stroke, nhưng đủ chính xác để canh giữa
...
def generate(row):
    ...
    # === PHÂN VÙNG MỚI ===
    left_w  = int(img_w * layout["right_prop"])  # panel NV (TRÁI)
    right_w = img_w - left_w                      # panel TEXT/HOOK (PHẢI)
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA1-T6.jpg"
    try:
    ...
    # overlay riêng cho panel PHẢI (nơi đặt TEXT/HOOK)
    right_panel_bg = ImageOps.fit(bg_img, (right_w, img_h), method=Image.LANCZOS, centering=(1.0, 0.5))
    canvas.paste(right_panel_bg, (left_w, 0))  # dán vào phía PHẢI

    # --- ẢNH NHÂN VẬT (NV) ---
    src = Image.open(bg_path)

    # Tùy bật/tắt tách nền (nếu bạn đã thêm công tắc enable_cutout)
    if layout.get("enable_cutout", False):
        cut = smart_cutout(src).convert("RGBA")
    else:
    ...
    # === ÉP CAO = CHIỀU CAO THUMB ===
    img_w, img_h = layout["img_width"], layout["img_height"]
    left_w  = int(img_w * layout["right_prop"])   # panel NV TRÁI
    right_w = img_w - left_w

    # scale theo chiều cao
    orig_w, orig_h = cut.size
    scale = img_h / float(orig_h)
    new_w = int(orig_w * scale)
    cut = cut.resize((new_w, img_h), Image.LANCZOS)   # giờ cut có chiều cao = img_h

    # Căn giữa trong panel TRÁI, dán từ đỉnh (py=0)
    px = (left_w - new_w) // 2
    py = 0

    # Cắt phần vượt panel TRÁI nếu có (giữ đúng khung [0, left_w] x [0, img_h])
    dst_left, dst_top, dst_right, dst_bottom = 0, 0, left_w, img_h
    src_x0 = max(0, dst_left - px)
    src_y0 = max(0, dst_top  - py)
    src_x1 = min(new_w, dst_right - px)
    src_y1 = min(img_h, dst_bottom - py)

    if src_x1 > src_x0 and src_y1 > src_y0:
        region  = cut.crop((src_x0, src_y0, src_x1, src_y1))
        paste_x = max(px, dst_left)
        paste_y = max(py, dst_top)  # = 0
        canvas.paste(region, (paste_x, paste_y), region)


    # === TEXT + HOOK Ở BÊN PHẢI ===
    ...
    # khổ chữ tối đa trong panel PHẢI
    right_margin_left = layout.get("right_panel_left_margin", layout["left_margin_text"])
    max_w_txt = right_w - layout["margin_x"] - right_margin_left
    ...
    # bắt đầu vẽ ở panel PHẢI → cộng offset = left_w
    y = text_top + max(0, (max_h_txt - char_h_real) // 2)
    x_panel_offset = left_w
    ...
    for ln in lines:
        ...
        x = x_panel_offset + right_margin_left + (max_w_txt - w_line) // 2
        draw_rich_line(draw, x, y, segs, font)
        y += font.size + dyn_sp
        glb += len(ln) + 1

    # --- HOOK (pill/text) trong panel PHẢI ---
    kw_text   = clean_text(row.hook).upper()
    max_w_bar = right_w - 2 * layout["bar_padding"]
    bar_h     = img_h // 6
    bar_top   = img_h - bar_h
    safe_ratio = 0.70
    ...
    if best_font:
        kw_font = best_font
        # tâm pill/ chữ hook nằm giữa panel PHẢI
        kw_x = left_w + right_w // 2
        OFFSET_Y = 30
        kw_y = bar_top + bar_h // 2 - OFFSET_Y

        # vẽ chữ hook 1 lần (như trước)
        draw.text(
            (kw_x, kw_y),
            kw_text,
            fill=layout.get("hook_text_color", layout["keyword_color"]),
            font=kw_font,
            anchor="mm",
            stroke_width=layout.get("keyword_stroke", 0) if layout.get("hook_text_stroke", True) else 0,
            stroke_fill=layout.get("hook_text_stroke_color", layout.get("keyword_stroke_color")),
        )

        if layout.get("hook_bg_enabled", True):
            ...
            RADIUS     = 20
            EDGE_W     = 6
            HOOK_BG    = layout["hook_bg_color"]
            HOOK_EDGE  = layout.get("hook_edge_color", layout["text_bg_color"])

            # giới hạn trong panel PHẢI: [left_w, img_w]
            x0 = max(left_w, kw_x - (txt_w // 2 + PILL_PAD_X))
            y0 = kw_y - (txt_h // 2 + PILL_PAD_Y)
            x1 = min(img_w, kw_x + (txt_w // 2 + PILL_PAD_X))
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA1-T6", na=False)]
