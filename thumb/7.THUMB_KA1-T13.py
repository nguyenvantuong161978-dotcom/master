...
layout = {
    ...
    "keyword_color": "#f8f600",       # màu chữ hook (đỏ tươi)
    "keyword_stroke": 1,              # độ dày viền chữ hook
    "keyword_stroke_color": "#f8f600",# màu viền chữ hook (hồng nhạt)

    # 🎨 Màu hook bar (pill dưới cùng)
    "hook_bg_color": "#000000",           # nền pill hook (vàng)
    "hook_edge_color": "#000000",         # viền pill hook (xanh dương đậm)
    "hook_text_color": "#f8f600",         # màu chữ hook (đỏ tươi)
    "hook_text_stroke_color": "#f8f600",  # màu viền chữ hook (hồng nhạt)
...
def generate(row):
    ...
    # --- Nền full (phủ trước) ---
    text_bg_img_path = ASSETS_DIR / "KA1-T13.jpg"
    try:
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA1-T13", na=False)]
