...
def smart_cutout(pil_image: Image.Image,
    ...
    """Trả về ảnh RGBA có alpha mềm. Ưu tiên MODNet; fallback rembg."""
    im = pil_image
...
def generate(row):
    ...
    cut = smart_cutout(src)  # RGBA với alpha mềm
    ...
    # Scale để vừa trong panel phải (chọn scale nhỏ hơn để không bị cắt)    # Scale theo chiều cao để nhân vật to như KA2
...
def load_df():
    ...
    df = df[df["channel"].str.contains("KA4-T1", na=False)]
